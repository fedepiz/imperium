...
world_update_render_terrain :: proc() {
	...
	for terrain, i in WORLD.atlas.terrain {
		...
		rt.coast[i] = math.lerp(coast_side[i] * near, far, math.smoothstep(COAST_REACH - 1, COAST_REACH, near))
	}
	...
	for dy in -1 ..= 1 {
		for dx in -1 ..= 1 {
			...
			if WORLD.atlas.terrain[y * WORLD_WIDTH + x].surface in WATER {
				polylines_add(lines, [2]f32{f32(cell.x), f32(cell.y)} + 0.5 + [2]f32{f32(dx), f32(dy)} * 0.75)
				return
			}
		}
	}
...
world_trace_coasts :: proc(lines: ^Polylines) {
	...
	is_water :: proc(x, y: int) -> bool {return WORLD.atlas.terrain[y * WORLD_WIDTH + x].surface in WATER}
