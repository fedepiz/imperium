...
ui_width_push :: proc(ctx: ^UI_Ctx, value: UI_Size) {ui_property_push(
		&ctx.ui.properties.width,
		value,
	)}
ui_width_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.width)}
ui_width_next :: proc(ctx: ^UI_Ctx, value: UI_Size) {ui_property_next(
		&ctx.ui.properties.width,
		value,
	)}
ui_height_push :: proc(ctx: ^UI_Ctx, value: UI_Size) {ui_property_push(
		&ctx.ui.properties.height,
		value,
	)}
ui_height_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.height)}
ui_height_next :: proc(ctx: ^UI_Ctx, value: UI_Size) {ui_property_next(
		&ctx.ui.properties.height,
		value,
	)}
ui_size_min_push :: proc(ctx: ^UI_Ctx, value: [2]f32) {ui_property_push(
		&ctx.ui.properties.size_min,
		value,
	)}
ui_size_min_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.size_min)}
ui_size_min_next :: proc(ctx: ^UI_Ctx, value: [2]f32) {ui_property_next(
		&ctx.ui.properties.size_min,
		value,
	)}
ui_position_push :: proc(ctx: ^UI_Ctx, value: [2]f32) {ui_property_push(
		&ctx.ui.properties.position,
		value,
	)}
ui_position_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.position)}
ui_position_next :: proc(ctx: ^UI_Ctx, value: [2]f32) {ui_property_next(
		&ctx.ui.properties.position,
		value,
	)}
ui_padding_push :: proc(ctx: ^UI_Ctx, value: [2]f32) {ui_property_push(
		&ctx.ui.properties.padding,
		value,
	)}
ui_padding_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.padding)}
ui_padding_next :: proc(ctx: ^UI_Ctx, value: [2]f32) {ui_property_next(
		&ctx.ui.properties.padding,
		value,
	)}
...
ui_axis_push :: proc(ctx: ^UI_Ctx, value: UI_Axis) {ui_property_push(
		&ctx.ui.properties.axis,
		value,
	)}
ui_axis_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.axis)}
ui_axis_next :: proc(ctx: ^UI_Ctx, value: UI_Axis) {ui_property_next(
		&ctx.ui.properties.axis,
		value,
	)}
ui_font_push :: proc(ctx: ^UI_Ctx, value: Font_Id) {ui_property_push(
		&ctx.ui.properties.font,
		value,
	)}
ui_font_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.font)}
ui_font_next :: proc(ctx: ^UI_Ctx, value: Font_Id) {ui_property_next(
		&ctx.ui.properties.font,
		value,
	)}
ui_radius_push :: proc(ctx: ^UI_Ctx, value: f32) {ui_property_push(
		&ctx.ui.properties.radius,
		value,
	)}
ui_radius_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.radius)}
ui_radius_next :: proc(ctx: ^UI_Ctx, value: f32) {ui_property_next(
		&ctx.ui.properties.radius,
		value,
	)}
ui_thickness_push :: proc(ctx: ^UI_Ctx, value: f32) {ui_property_push(
		&ctx.ui.properties.thickness,
		value,
	)}
ui_thickness_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.thickness)}
ui_thickness_next :: proc(ctx: ^UI_Ctx, value: f32) {ui_property_next(
		&ctx.ui.properties.thickness,
		value,
	)}
ui_softness_push :: proc(ctx: ^UI_Ctx, value: f32) {ui_property_push(
		&ctx.ui.properties.softness,
		value,
	)}
ui_softness_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.softness)}
ui_softness_next :: proc(ctx: ^UI_Ctx, value: f32) {ui_property_next(
		&ctx.ui.properties.softness,
		value,
	)}
ui_opacity_push :: proc(ctx: ^UI_Ctx, value: f32) {ui_property_push(
		&ctx.ui.properties.opacity,
		value,
	)}
ui_opacity_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.opacity)}
ui_opacity_next :: proc(ctx: ^UI_Ctx, value: f32) {ui_property_next(
		&ctx.ui.properties.opacity,
		value,
	)}
ui_text_align_push :: proc(ctx: ^UI_Ctx, value: UI_Align) {ui_property_push(
		&ctx.ui.properties.text_align,
		value,
	)}
ui_text_align_pop :: proc(ctx: ^UI_Ctx) {ui_property_pop(&ctx.ui.properties.text_align)}
ui_text_align_next :: proc(ctx: ^UI_Ctx, value: UI_Align) {ui_property_next(
		&ctx.ui.properties.text_align,
		value,
	)}

// Every color role has its own stack. Literal colors are animation targets, like theme colors.
ui_color_push :: proc(ctx: ^UI_Ctx, role: UI_Color, value: [4]f32) {ui_property_push(
		&ctx.ui.properties.colors[role],
		UI_Color_Binding{literal = true, value = value},
	)}
ui_color_pop :: proc(ctx: ^UI_Ctx, role: UI_Color) {ui_property_pop(
		&ctx.ui.properties.colors[role],
	)}
ui_color_next :: proc(ctx: ^UI_Ctx, role: UI_Color, value: [4]f32) {ui_property_next(
		&ctx.ui.properties.colors[role],
		UI_Color_Binding{literal = true, value = value},
	)}
// Source None restores theme inheritance for the destination role within this scope/box.
ui_color_role_push :: proc(ctx: ^UI_Ctx, role, source: UI_Color) {ui_property_push(
		&ctx.ui.properties.colors[role],
		UI_Color_Binding{role = source},
	)}
ui_color_role_next :: proc(ctx: ^UI_Ctx, role, source: UI_Color) {ui_property_next(
		&ctx.ui.properties.colors[role],
		UI_Color_Binding{role = source},
	)}
...
ui_box_make :: proc(
	...
	if id != 0 {
		...
		for role in UI_Color {
			...
			b.colors_target[role] =
				binding.value if binding.literal else ui_theme_color(ui, b.tags, source)
		}
...
ui_hit :: proc(ctx: ^UI_Ctx, pos: [2]f32, scroll: bool = false) -> (id: UI_Box_Id) {
	...
	for b, i in ctx.ui.boxes {
		interactive :=
			(.Scroll_X in b.flags || .Scroll_Y in b.flags) if scroll else (.Clickable in b.flags || .Movable in b.flags || .Scroll_X in b.flags || .Scroll_Y in b.flags)
		if interactive &&
		   ui_surface_accepts_input(ctx.ui, b) &&
		   b.order > order &&
	...
	for b, i in ui.boxes {
		if .Focusable in b.flags && ui_surface_accepts_input(ui, b) {
			...
			case:
				...
				delta := [2]f32 {
					b.rect.x + b.rect.z / 2 - origin.x - origin.z / 2,
					b.rect.y + b.rect.w / 2 - origin.y - origin.w / 2,
				}
				axis := 0 if action == .Left || action == .Right else 1
				direction: f32 =
					-1 if action == .Left || action == .Up || action == .Page_Up else 1
				distance := abs(delta[axis]) + 2 * abs(delta[1 - axis])
				if id != current &&
				   delta[axis] * direction > 0 &&
				   distance < best_distance {best, best_distance = id, distance}
			}
		}
	}
	if best == 0 {best = wrap}
...
ui_interact :: proc(ctx: ^UI_Ctx) {
	...
	for event in ui.events {
		...
		case .Press:
			...
			if id != 0 {
				...
				if event.button == 1 {
					...
					ui.scroll_drag = 0
					if .Scroll_Y in b.flags &&
					   event.pos.x >= b.rect.x + b.rect.z - 14 {ui.scroll_drag = 2}
					if .Scroll_X in b.flags &&
					   event.pos.y >= b.rect.y + b.rect.w - 14 {ui.scroll_drag = 1}
				}
		...
		case .Key:
			...
			case .Accept:
				if id != 0 &&
				   ui_surface_accepts_input(
					   ui,
					   ui.boxes[id],
				   ) {ui.signals[id].flags += {.Pressed, .Released, .Clicked, .Keyboard}}
			case .Left, .Right, .Up, .Down, .Home, .End, .Page_Up, .Page_Down:
				if id != 0 && .Adjustable in ui.boxes[id].flags {
					#partial switch event.action {
					case .Left:
						ui.signals[id].nav.x -= 1
					case .Right:
						ui.signals[id].nav.x += 1
					case .Up:
						ui.signals[id].nav.y -= 1
					case .Down:
						ui.signals[id].nav.y += 1
					case .Home:
						ui.signals[id].nav = {-100000, -100000}
					case .End:
						ui.signals[id].nav = {100000, 100000}
					case .Page_Up:
						ui.signals[id].nav.y -= 10
					case .Page_Down:
						ui.signals[id].nav.y += 10
					case:
					}
...
ui_box_padding :: proc(b: UI_Box) -> [2]f32 {
	return(
		b.padding * 2 +
		[2]f32{12 if .Scroll_Y in b.flags else 0, 12 if .Scroll_X in b.flags else 0} \
	)
}
	...
	for axis in 0 ..< 2 {
		...
		for id in ui.order {
			if id != 0 {
				...
				case .Parent:
					parent := b.parent
					for parent != 0 && ui.boxes[parent].size[axis].kind == .Children {
						parent = ui.boxes[parent].parent
					}
					if parent != 0 {
						space :=
							ui.boxes[parent].rect[axis + 2] -
							ui_box_padding(ui.boxes[parent])[axis]
						size = max(0, space) * pref.value
					}
				case .Children, .Fill:
				}
				...
				if b.size[axis].kind == .Children {
					...
					for child := b.child_first; child != 0; child = ui.boxes[child].sibling_next {
						...
						if .Floating not_in c.flags {
							if int(b.axis) ==
							   axis {size += c.rect[axis + 2]} else {size = max(size, c.rect[axis + 2])}
							count += 1
						}
					}
				...
				for child := b.child_first; child != 0; child = ui.boxes[child].sibling_next {
					...
					if .Floating not_in c.flags {
						...
						if c.size[axis].kind == .Parent && b.size[axis].kind != .Children {
							c.rect[axis + 2] = max(
								c.size_min[axis],
								available * c.size[axis].value,
							)
						}
						if c.size[axis].kind ==
						   .Fill {weights += max(0, c.size[axis].value)} else {total += c.rect[axis + 2]}
						flexible +=
							max(0, c.rect[axis + 2] - c.size_min[axis]) *
							(1 - clamp(c.size[axis].strictness, 0, 1))
					}
				}
				if along {total += f32(max(0, count - 1)) * b.gap}
				shrink :=
					clamp((total - available) / flexible, 0, 1) if flexible > 0 && !overflow else 0
				for child := b.child_first; child != 0; child = ui.boxes[child].sibling_next {
					c := &ui.boxes[child]
					if .Floating not_in c.flags {
						size := c.rect[axis + 2]
						if c.size[axis].kind == .Fill {
							size =
								max(0, available - total) * c.size[axis].value / weights if along && weights > 0 else available
						} else if along {
							size -=
								max(0, size - c.size_min[axis]) *
								(1 - clamp(c.size[axis].strictness, 0, 1)) *
								shrink
	...
	for id in ui.order {
		if id != 0 {
			...
			for child := b.child_first; child != 0; child = ui.boxes[child].sibling_next {
				...
				if .Floating in
				   c.flags {pos = c.position} else {pos[axis] += cursor; cursor += c.rect[axis + 2] + b.gap}
				...
				if c.surface != .Main && .Floating in c.flags {
					pos = {
						clamp(pos.x, 0, max(0, ctx.view_size.x - c.rect.z)),
						clamp(pos.y, 0, max(0, ctx.view_size.y - c.rect.w)),
					}
				}
...
ui_checkbox :: proc(ctx: ^UI_Ctx, label: string, value: ^bool) -> UI_Signal {
	...
	if s.box !=
	   0 {ctx.ui.boxes[s.box].draw_kind = .Checkbox; ctx.ui.boxes[s.box].value.x = 1 if value^ else 0}
	return s
}
		...
		for part in parts {
			if ctx.ui.run_next < UI_RUN_MAX {
				...
				if part.is_image && size.x == 0 {
					...
					size.x =
						size.y * region.source.z / region.source.w if region.source.w > 0 else 0
				}
				ctx.ui.runs[ctx.ui.run_next] = {
					ui_text_store(ctx.ui, part.text),
					part.font,
					part.color,
					part.image,
					size,
...
ui_row_begin :: proc(ctx: ^UI_Ctx, key: string = "") -> UI_Box_Id {return ui_group_begin(
		ctx,
		key,
		.X,
		{},
...
ui_col_begin :: proc(ctx: ^UI_Ctx, key: string = "") -> UI_Box_Id {return ui_group_begin(
		ctx,
		key,
		.Y,
		{},
...
ui_slider :: proc(
	...
	id := ui_box_make(
		ctx,
		ui_key_from_string(ui_seed(ctx), label),
		{.Background, .Border, .Text, .Clickable, .Focusable, .Adjustable},
		style,
	)
	ui_box_text(
		ctx,
		id,
		label[:strings.index(label, "##")] if strings.index(label, "##") >= 0 else label,
	)
	ui_tag_pop(ctx)
	s := ui_signal(ctx, id)
	if id != 0 {
		b := &ctx.ui.boxes[id]
		previous := value^
		if (.Pressed in s.flags || .Dragging in s.flags) &&
		   .Keyboard not_in s.flags &&
		   b.rect.z > 0 {
			value^ =
				low +
				clamp((s.mouse.x - b.rect.x - 10) / max(1, b.rect.z - 20), 0, 1) * (high - low)
		}
...
ui_scrollbar :: proc(
	...
	id := ui_box_make(
		ctx,
		ui_key_from_string(ui_seed(ctx), key),
		{.Clickable, .Focusable, .Adjustable},
		style,
	)
	s := ui_signal(ctx, id)
	if id != 0 {
...
ui_pane_begin :: proc(ctx: ^UI_Ctx, key: string, rect: ^[4]f32) -> UI_Box_Id {
	...
	id := ui_box_make(
		ctx,
		ui_key_from_string(ui_seed(ctx), key),
		{.Background, .Border, .Clip, .Floating, .Movable, .Shadow},
		style,
	)
	if id != 0 {
		s := ui_signal(ctx, id)
		...
		if .Dragging in s.flags || .Released in s.flags {
			start := b.drag_rect
			if ctx.ui.drag_start.x > start.x + start.z - 18 &&
			   ctx.ui.drag_start.y > start.y + start.w - 18 {
...
ui_popup_begin :: proc(ctx: ^UI_Ctx, key: string, width: f32 = 240) -> bool {
	...
	id := ui_box_make(
		ctx,
		popup_key,
		{.Background, .Border, .Floating, .Clip, .Shadow, .Clickable},
		style,
	)
	ui_parent_push(ctx, id)
	return true
...
ui_tooltip :: proc(ctx: ^UI_Ctx, anchor: UI_Box_Id, text: string, width: f32 = 260) {
	if anchor != 0 &&
	   ctx.ui.boxes[anchor].key == ctx.ui.hot &&
	   ctx.ui.hover_time >= 0.5 &&
	   ctx.ui.active == 0 {
		...
		id := ui_box_make(
			ctx,
			ui_key_from_string(ctx.ui.boxes[anchor].key, "###tooltip"),
			{.Background, .Border, .Floating, .Shadow},
			style,
		)
		ui_parent_push(ctx, id)
...
ui_table_begin :: proc(ctx: ^UI_Ctx, key: string, weights: []f32) -> UI_Box_Id {
	...
	if depth < UI_TABLE_DEPTH_MAX {
		...
		t^ = {
			column_len = min(len(weights), UI_TABLE_COLUMNS_MAX),
		}
		for weight, i in weights[:t.column_len] {t.weights[i] = max(weight, 0); t.weight_sum += t.weights[i]}
	}
	return ui_col_begin(ctx, key)
}
ui_table_end :: proc(ctx: ^UI_Ctx) {ui_col_end(ctx); ctx.ui.table_depth = max(
		0,
		ctx.ui.table_depth - 1,
	)}
ui_table_row_begin :: proc(ctx: ^UI_Ctx, key: string = "") -> UI_Box_Id {
	if ctx.ui.table_depth > 0 &&
	   ctx.ui.table_depth <=
		   UI_TABLE_DEPTH_MAX {ctx.ui.table_stack[ctx.ui.table_depth - 1].column_next = 0}
	ui_gap_push(ctx, 0)
...
ui_table_cell_begin :: proc(ctx: ^UI_Ctx) -> UI_Box_Id {
	...
	if ctx.ui.table_depth > 0 && ctx.ui.table_depth <= UI_TABLE_DEPTH_MAX {
		...
		if t.column_next < t.column_len &&
		   t.weight_sum > 0 {weight = t.weights[t.column_next] / t.weight_sum}
		t.column_next += 1
	}
...
ui_rgb_hsv :: proc(rgb: [3]f32) -> [3]f32 {
	...
	if d > 0 {
		if hi ==
		   rgb.x {h = (rgb.y - rgb.z) / d} else if hi == rgb.y {h = 2 + (rgb.z - rgb.x) / d} else {h = 4 + (rgb.x - rgb.y) / d}
		h /= 6
		if h < 0 {h += 1}
	}
...
ui_color_picker :: proc(ctx: ^UI_Ctx, key: string, color: ^[4]f32) -> (signal: UI_Signal) {
	...
	for kind in 0 ..< 3 {
		...
		id := ui_box_make(
			ctx,
			ui_key_from_string(ui_seed(ctx), name),
			{.Border, .Clickable, .Focusable, .Adjustable},
			style,
		)
		controls[kind] = id
		if id != 0 {
			...
			if (.Pressed in s.flags || .Dragging in s.flags) && .Keyboard not_in s.flags {
				p := [2]f32 {
					clamp((s.mouse.x - b.rect.x) / max(1, b.rect.z), 0, 1),
					clamp((s.mouse.y - b.rect.y) / max(1, b.rect.w), 0, 1),
				}
			...
			if kind ==
			   0 {hsv.y = clamp(hsv.y + f32(s.nav.x) * 0.01, 0, 1); hsv.z = clamp(hsv.z - f32(s.nav.y) * 0.01, 0, 1)}
...
ui_draw_text :: proc(ctx: ^UI_Ctx, b: UI_Box, position: [2]f32, width: f32, color: [4]f32) {
	...
	if b.runs.len > 0 {
		...
		for run in ctx.ui.runs[b.runs.begin:b.runs.begin + b.runs.len] {
			ascent = max(
				ascent,
				run.size.y if run.is_image else ctx.draw.sprites.fonts[run.font].info.ascent,
			)
		}
		x := position.x
		for run in ctx.ui.runs[b.runs.begin:b.runs.begin + b.runs.len] {
			...
			if run.is_image {
				draw_image(
					ctx.draw,
					run.image,
					{x, position.y + ascent - run.size.y, run.size.x, run.size.y},
					tint,
				)
				x += run.size.x
			} else {
				size := draw_text(
					ctx.draw,
					run.font,
					ui_text_get(ctx.ui, run.text),
					{x, position.y + ascent - ctx.draw.sprites.fonts[run.font].info.ascent},
					tint,
				)
				x += size.x
			}
		...
		for ch, offset in text {
			amount: f32
			if sprite, ok := sprite_of_glyph(ctx.draw.sprites, b.font, ch);
			   ok {amount = ctx.draw.sprites.glyphs[sprite].advance}
			if ch == '\n' || advance + amount + trailer > width {end = offset; break}
			advance += amount
		}
		draw_text(ctx.draw, b.font, text[:end], position, color)
		if trailer <=
		   width {draw_text(ctx.draw, b.font, "...", position + [2]f32{advance, 0}, color)}
	} else {draw_text(ctx.draw, b.font, text, position, color)}
}

@(private = "file")
ui_draw_scrollbar :: proc(
	draw: ^Draw_Ctx,
	rect: [4]f32,
	axis: int,
	offset, fraction: f32,
...
ui_paint :: proc(ctx: ^UI_Ctx) {
	...
	for id in ctx.ui.order {
		if id != 0 {
			...
			if .Background in
			   b.flags {draw_rectangle(ctx.draw, b.rect, bg, radius = b.radius, softness = b.softness)}
			if .Border in
			   b.flags {draw_rectangle_lines(ctx.draw, b.rect, palette[.Border], b.thickness, radius = b.radius, softness = b.softness)}
			content := [4]f32 {
				b.rect.x + b.padding.x,
				b.rect.y + b.padding.y,
				max(0, b.rect.z - ui_box_padding(b).x),
				max(0, b.rect.w - ui_box_padding(b).y),
			}
			if .Image in
			   b.flags {draw_image(ctx.draw, b.image, content, {1, 1, 1, b.opacity * (1 - b.disabled_t * 0.5)})}
			...
			case .Checkbox, .Expander:
				...
				if b.draw_kind == .Checkbox {
					if b.value.x >
					   0 {draw_rectangle(ctx.draw, {r.x + 4, r.y + 4, d - 8, d - 8}, palette[.Focus], radius = 2)}
				} else {
					draw_rectangle(ctx.draw, {r.x + 4, r.y + d / 2 - 1, d - 8, 2}, palette[.Text])
					if b.value.x ==
					   0 {draw_rectangle(ctx.draw, {r.x + d / 2 - 1, r.y + 4, 2, d - 8}, palette[.Text])}
				}
				content.x += d + 8
			...
			case .Slider:
				...
				draw_rectangle(
					ctx.draw,
					{r.x, r.y, r.z * b.value.x, r.w},
					palette[.Focus],
					radius = 2,
				)
				draw_rectangle(
					ctx.draw,
					{r.x + r.z * b.value.x - 5, r.y - 4, 10, 12},
					palette[.Text],
					radius = 4,
					softness = 0.5,
				)
			...
			case .Saturation_Value:
				...
				ui_draw_gradient(
					...
					b.rect,
					{
						.Top_Left = {1, 1, 1, b.opacity},
						.Top_Right = {hue.x, hue.y, hue.z, b.opacity},
						.Bot_Left = {0, 0, 0, b.opacity},
						.Bot_Right = {0, 0, 0, b.opacity},
					},
				)
				draw_rectangle_lines(
					ctx.draw,
					{
						b.rect.x + b.value.y * b.rect.z - 4,
						b.rect.y + (1 - b.value.z) * b.rect.w - 4,
						8,
						8,
					},
					{1, 1, 1, 1},
			...
			case .Hue:
				for segment in 0 ..< 6 {
					a, b_color :=
						ui_hsv_rgb({f32(segment) / 6, 1, 1}),
						ui_hsv_rgb({f32(segment + 1) / 6, 1, 1})
					left, right :=
						[4]f32{a.x, a.y, a.z, b.opacity},
						[4]f32{b_color.x, b_color.y, b_color.z, b.opacity}
					ui_draw_gradient(
						ctx.draw,
						{b.rect.x + f32(segment) * b.rect.z / 6, b.rect.y, b.rect.z / 6, b.rect.w},
						{
							.Top_Left = left,
							.Bot_Left = left,
							.Top_Right = right,
				...
				draw_rectangle_lines(
					ctx.draw,
					{b.rect.x + b.value.x * b.rect.z - 3, b.rect.y, 6, b.rect.w},
					{1, 1, 1, 1},
					1,
				)
			case .Alpha:
				for row in 0 ..< 2 {
					for col in 0 ..< 16 {
						...
						draw_rectangle(
							ctx.draw,
							{
								b.rect.x + f32(col) * b.rect.z / 16,
								b.rect.y + f32(row) * b.rect.w / 2,
								b.rect.z / 16,
								b.rect.w / 2,
							},
							{v, v, v, b.opacity},
				...
				ui_draw_gradient(
					ctx.draw,
					b.rect,
					{.Top_Left = a, .Bot_Left = a, .Top_Right = z, .Bot_Right = z},
				)
				draw_rectangle_lines(
					ctx.draw,
					{b.rect.x + b.value.w * b.rect.z - 3, b.rect.y, 6, b.rect.w},
					{1, 1, 1, 1},
					1,
				)
			case .Swatch:
			...
			if .Text in b.flags {
				...
				align: f32 =
					0 if b.text_align == .Start else (0.5 if b.text_align == .Center else 1)
				pos := [2]f32 {
					content.x + max(0, content.z - dim.x) * align,
					content.y + max(0, content.w - dim.y) / 2,
				}
			...
			for axis in 0 ..< 2 {
				...
				if scroll {
					...
					r :=
						[4]f32{b.rect.x + b.padding.x, b.rect.y + b.rect.w - 10, view, 6} if axis == 0 else [4]f32{b.rect.x + b.rect.z - 10, b.rect.y + b.padding.y, 6, view}
					ui_draw_scrollbar(
						ctx.draw,
						r,
						axis,
...
ui_end :: proc(ctx: ^UI_Ctx) {
	...
	for &b in ui.boxes {
		if b.frame == ui.frame {
			...
			b.active_t +=
				((1 if (b.key != 0 && b.key == ui.active) || .Selected in b.flags else 0) -
					b.active_t) *
				rate
	...
	if ui.popup_key != 0 {
		...
		if ui.popup_key != 0 && (focus == 0 || ui.boxes[focus].surface != .Popup) {
			for id in ui.order {
				if id != 0 &&
				   ui.boxes[id].surface == .Popup &&
				   .Focusable in ui.boxes[id].flags &&
				   .Disabled not_in ui.boxes[id].flags {
